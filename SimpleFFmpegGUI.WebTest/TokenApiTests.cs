...
public class TokenApiTests(SimpleFFmpegWebApplicationFactory factory) : SimpleFFmpegApiTestsBase(factory)
{
    ...
    {
        var client = factory.CreateClient();
        var request = new HttpRequestMessage(HttpMethod.Get, "/api/Task");
    ...
    public async Task TestTokenNeedAsync()
    {
        ...
        var need = await GetObjectFromJsonAsync<bool>("/api/Token/Need");
        need.Should().BeTrue();
    }
    ...
    {
        // 有效的Token（明文）
        var valid = await GetObjectFromJsonAsync<bool>("/api/Token/Check/Test_Token_123");
        valid.Should().BeTrue();

        // 无效的Token
        var invalid = await GetObjectFromJsonAsync<bool>("/api/Token/Check/wrong_token");
